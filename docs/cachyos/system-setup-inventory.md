...
| `/etc/systemd/journald.conf.d/90-retention.conf` | `[Journal]` / `SystemMaxUse=2G` / `MaxRetentionSec=1month`; then `sudo systemctl restart systemd-journald`. **This deliberately overrides a CachyOS default** — `cachyos-settings` ships `/usr/lib/systemd/journald.conf.d/00-journal-size.conf` with `SystemMaxUse=50M`. Not a bug being fixed: a considered trade reversed. 50 MB held under a day of logs here (oldest entry was newer than the current boot), which silently defeats the weekly MCE check and post-freeze `journalctl -b -1`. Cost is disk, which this machine has. Applied desktop 2026-08-03 | `todo.md` (surge check, parked freeze) | universal — apply on laptop too |

...
- ✅ **Desktop — RGB outage RESOLVED 2026-08-04: BIOS `LED lighting` was `Aura Off`.** Full mechanism in [nvidia/openrgb.md](nvidia/openrgb.md). **Retracted:** the OpenRGB `1.0rc2→rc3` regression theory and its planned pacman-cache downgrade test — there was never an OpenRGB fault, and the "writes accepted, nothing visible" symptom that made rc3 look guilty was the hardware gate. Do not re-run that test. Software side was correct throughout: `openrgb 1.0rc3`, `~/.config/autostart/openrgb-apply-profile.desktop` present and enabled, byte-identical to its dotfiles source. **All three follow-ups closed 2026-08-06:** plain `openrgb -p` does apply (`--server` not needed — measured via the sleep hook, see [nvidia/openrgb.md](nvidia/openrgb.md)); `~/.config/OpenRGB/my profile.orp` is saved and its colours are tabled in [peripherals/openrgb.md](peripherals/openrgb.md); the redundant `.desktop.bak` is deleted.
- ✅ **Desktop — RGB not restored after sleep: RESOLVED 2026-08-06, and it was never an OpenRGB fault either.** The sleep hook was installed to `/etc/systemd/system-sleep/`, which **systemd does not scan** — it reads `/usr/lib/systemd/system-sleep/` only, so the hook never executed once. Moved there (dotfiles `DEPLOYED_COPIES`), verified end-to-end. Mechanism, the measured resume timeline, and the four claims this retracted: [peripherals/openrgb.md](peripherals/openrgb.md).

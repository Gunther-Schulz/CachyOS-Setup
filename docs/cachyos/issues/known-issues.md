...
Open / minor issues without a full fix. Resolved items with a real fix live in
...
**Double key press in GRUB menu:** Seen on 4K Acer; disconnecting it forces the display to the Dell 2K where input is slow but works. Try: lower `GRUB_GFXMODE` (e.g. `1280x1024x32`) in `/etc/default/grub`, `sudo update-grub`; disable CSM in BIOS.
